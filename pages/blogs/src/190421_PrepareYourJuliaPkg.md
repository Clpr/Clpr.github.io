...
|Interpolations | 0.11.2 | Numerical | Interpolations; developed in pure Julia |
|Dierckx | 0.4.1 | Numerical | Spline interpolations; a wrapper of Fortran's Dierckx. has much fewer methods than `Interpolations` but more convenient if you do not need other interpolation algorithms |
|MultivariateStats | 0.6.0 | Stats |Matrix-based multivariate regressions|

`Pkg` is another package once popular before Julia 1.0. In fact, the current pkg mode is an integrated `Pkg` package. Users do not need to `add Pkg` since Julia 1.0 unless they want to run some old codes written in Julia 0.6 or before where `Pkg` used.
