...
class WorkExp_teaching(ExpRecord):
    ...
    def new(self, idx:str, flag_required : bool = False ):
        ...
        """
        # 首先构建第一行的标签们
        tmplabels = [ 'Title', 'Institution', 'Department', 'Since', 'Until', 'Until Now?' ]
        # 以及对应的问题names（数据字段名，keys）
        tmpnames = [ 'title', 'univ', 'department', 'sinceyear', 'untilyear', 'untilnow' ]
        tmpnames = [ 'workexp_teach_' + idx + '_' + x for x in tmpnames ] # 使用给定的idx产生对当前记录特定的names，如'workexp1_title'
        ...
        # 然后生成第二行
        tmp2ndRow = []
        # 2.0 title，使用公用字符串列表li_worktitle_teach和方法newSelect
        tmp2ndRow.append( '<td>' + 
