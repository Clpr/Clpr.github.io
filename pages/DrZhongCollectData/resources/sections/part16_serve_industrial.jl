...
# 1. header <h2>
add!(tmpdiv, tag_h2("XVI. SERVE: INDUSTRIAL 兼职业界工作"))
add!(tmpdiv, tag_h3("NOTE:"))
# 2. notes <ol>
add!(tmpdiv, quicktag_ol(String[
    "兼职业界工作指该教师兼职于业界企业、银行、投行、咨询公司等担任学术或行政职位的经历。",
...
tmpmat = predefvars.empty_tablematrix(3 * RcdNum + 1,6)
    ...
    # ----------------------
    for idx in 1:RcdNum
        tmpmat[(idx*3-2):(idx*3),:] = templates.IndustrialPartTimeRecord( idx, required = false )
    end # idx
    # ---------------------- more records than asked?
    tmpmat[size(tmpmat,1),1:2] = templates.AskMoreRecord( "serve_industrial_morerecord" , required = true )
